import Joyride, { type CallBackProps, STATUS, type Step, type Styles } from 'react-joyride'
...
const STEPS: Step[] = [
  {
    ...
    content: 'Это главный экран — здесь хранятся все ваши квизы. Проведём короткий тур, чтобы вы освоились.',
    placement: 'bottom',
    disableBeacon: true,
  },
  {
    ...
    title: '➕ Создать квиз',
    content: 'Нажмите эту кнопку, чтобы создать новый квиз с нуля. Система сразу откроет редактор.',
    placement: 'bottom',
    disableBeacon: true,
  },
  {
    ...
    content: 'Каждый квиз отображается в виде карточки. Нажмите на неё, чтобы открыть редактор и изменить вопросы.',
    placement: 'right',
    disableBeacon: true,
  },
  {
    ...
    title: '▶ Превью',
    content: 'Проверьте квиз глазами ученика — полноэкранный режим с таймером, вариантами ответов и подсчётом баллов.',
    placement: 'top',
    disableBeacon: true,
  },
  {
    ...
    title: '📦 Экспорт',
    content: 'Сохраните квиз в файл .pack — его можно перенести на другие устройства или загрузить в Player для учеников.',
    placement: 'top',
    disableBeacon: true,
  },
  {
    ...
    title: '⚙️ Настройки',
    content: 'Здесь настраивается облачная синхронизация и ключ для AI-генерации фонов.',
    placement: 'right',
    disableBeacon: true,
  },
]
  ...
  return (
    <div
      ...
      style={{
        ...
        padding: '24px',
        maxWidth: '340px',
        boxShadow: '0 20px 60px rgba(0,0,0,0.5), 0 0 0 1px rgba(129,140,248,0.2)',
        color: '#fff',
    ...
    >
      ...
      {/* Title */}
      {step.title && (
        <h3 style={{ margin: '0 0 10px', fontSize: '1.05rem', fontWeight: 800, lineHeight: 1.3 }}>
          {step.title as string}
        </h3>
      )}

      {/* Content */}
      <p style={{ margin: '0 0 20px', fontSize: '0.9rem', lineHeight: 1.6, color: 'rgba(255,255,255,0.82)' }}>
...
const joyrideStyles: Partial<Styles> = {
  options: {
    ...
    spotlightShadow: '0 0 0 4px rgba(129,140,248,0.5), 0 0 40px rgba(129,140,248,0.25)',
    zIndex: 9999,
  },
}
...
export function AppTour() {
  ...
  async function handleCallback(data: CallBackProps) {
    ...
    const isFinished = [STATUS.FINISHED, STATUS.SKIPPED].includes(status as typeof STATUS.FINISHED)
    if (isFinished) {
      stopTour()
      // Навигация в редактор только при завершении (не при пропуске)
      if (status === STATUS.FINISHED) {
        try {
          const newId = await invoke<string>('create_quiz')
          // Pre-populate with sample questions of all main types
          const o1a = crypto.randomUUID(), o1b = crypto.randomUUID()
          const o1c = crypto.randomUUID(), o1d = crypto.randomUUID()
          const o2a = crypto.randomUUID(), o2b = crypto.randomUUID()
          const o2c = crypto.randomUUID(), o2d = crypto.randomUUID()
