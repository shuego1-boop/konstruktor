...
import {
  createQuizSchema,
  updateQuizSchema,
} from "@konstruktor/shared/schemas";
...
quizzesRoute.get("/:id", async (c) => {
  ...
  const [row] = await db
    ...
    .from(quizzes)
    .where(
      and(eq(quizzes.id, c.req.param("id")), eq(quizzes.teacherId, user.id)),
    );
...
quizzesRoute.post("/:id/share-code", async (c) => {
  ...
  if (existing.status !== "published")
    return c.json({ error: "Quiz must be published to share" }, 400);
...
quizzesRoute.get("/:id/sessions", async (c) => {
  ...
  // Per-question accuracy aggregates (from normalized question_answers table)
  let questionStats: {
    questionId: string;
    total: number;
    correct: number;
  ...
  if (totalSessions > 0) {
    ...
    const answerRows = await db
      .select({
        ...
        correct: sql<number>`cast(sum(case when ${questionAnswers.isCorrect} then 1 else 0 end) as int)`,
        avgTimeMs: sql<number>`cast(avg(${questionAnswers.responseTimeMs}) as int)`,
      })
      .from(questionAnswers)
      .where(inArray(questionAnswers.sessionId, sessionIds))
      .groupBy(questionAnswers.questionId);

    questionStats = answerRows.map((r) => ({
