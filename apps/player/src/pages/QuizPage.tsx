...
import type { Quiz, Question, QuizSession, SingleChoiceQuestion, MultipleChoiceQuestion, TrueFalseQuestion, MatchingQuestion, OrderingQuestion, HotspotQuestion } from '@konstruktor/shared'
import { QuizEngine } from '@konstruktor/quiz-engine'
import { TimerBar, ProgressBar, StreakBadge, AnswerFeedback, Spinner } from '@konstruktor/ui'
...
export function QuizPage() {
  ...
  async function loadQuiz() {
    ...
    try {
      ...
      startTimer((firstQ?.timeLimit ?? loaded.settings.timePerQuestion ?? 30) * 1000)
    } catch (err) {
  ...
  function submitAnswer() {
    ...
    if (currentQ.type === 'single_choice') {
      ...
      const pairs = mq.pairs.map((p) => ({ leftId: p.id, rightId: matchingMap[p.id] ?? '' }))
  ...
  async function handleNext() {
    ...
    if (engineState === 'completed') {
      ...
      const session: QuizSession = {
        ...
        ...(engineResult.completedAt !== undefined && { completedAt: engineResult.completedAt }),
        score: engineResult.score,
        totalPoints: engineResult.totalPoints,
        earnedPoints: engineResult.earnedPoints,
        ...(engineResult.isPassed !== undefined && { isPassed: engineResult.isPassed }),
      ...
      startTimer((nextQ?.timeLimit ?? quiz.settings.timePerQuestion ?? 30) * 1000)
      setPhase('question')
    }
  }

  if (loading) return <div className="flex min-h-screen items-center justify-center bg-slate-900"><Spinner size="lg" /></div>
  if (error) return <div className="flex min-h-screen items-center justify-center bg-slate-900 text-red-400">{error}</div>
  ...
  const streak = (() => {
    const answers = eng.getAnswers()
    let count = 0
    for (let i = answers.length - 1; i >= 0 && answers[i]!.isCorrect; i--) count++
    return count
  })()
  const timeLimit = (currentQ?.timeLimit ?? quiz.settings.timePerQuestion ?? 30) * 1000
  const isFeedback = phase === 'feedback'

  return (
    <div className="flex min-h-screen flex-col bg-slate-900 text-white">
      <header className="px-5 py-4 border-b border-slate-700 flex items-center gap-4">
        <button className="text-slate-400 hover:text-white" onClick={() => navigate('/home')}>←</button>
        <ProgressBar
      ...
      <main className="flex-1 flex flex-col p-5 max-w-2xl mx-auto w-full">
        ...
        {streak >= 2 && <StreakBadge streak={streak} className="mb-4 self-start" />}

        {currentQ && (
          <>
            <p className="text-xl font-semibold mb-6 leading-snug">{currentQ.text}</p>
                  ...
                  return (
                    ...
                    >
                      <span className={`mr-2 inline-block h-4 w-4 rounded border-2 align-middle ${
                        checked ? 'border-indigo-400 bg-indigo-400' : 'border-slate-500'
                      }`} />
                      {opt.text}
                    </button>
                  )
                ...
                <div className="flex flex-col gap-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="flex flex-col gap-2">
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Левая часть</p>
                      {mq.pairs.map((pair) => {
                        ...
                        return (
                          <button
                            ...
                            className={`rounded-xl border-2 px-4 py-3 text-left font-medium text-sm transition-colors min-h-[48px] ${
                              isActive ? 'border-indigo-400 bg-indigo-900'
                              : hasMatch ? 'border-green-600 bg-green-900/30'
                              : 'border-slate-600 bg-slate-800'
                            }`}
                          >
                            <span>{pair.left}</span>
                            {matchingMap[pair.id] && <span className="block text-xs text-green-400 mt-0.5">→ {matchingMap[pair.id]}</span>}
                          </button>
                        )
                      })}
                    </div>
                    <div className="flex flex-col gap-2">
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Правая часть</p>
                      {matchingShuffledRights.map((rightText) => {
                        ...
                        return (
                          <button
                            ...
                            className={`rounded-xl border-2 px-4 py-3 text-left font-medium text-sm transition-colors min-h-[48px] ${
                              isMappedToActive ? 'border-green-500 bg-green-900/30'
                              : isUsed ? 'border-slate-700 bg-slate-800/40 opacity-40'
                              : matchingActive ? 'border-indigo-500/50 bg-slate-800 hover:border-indigo-400'
                              : 'border-slate-600 bg-slate-800'
                            }`}
                          >
                            {rightText}
                  ...
                  {!isFeedback && allMapped && (
                    <button onClick={submitAnswer} className="w-full rounded-xl bg-indigo-600 py-4 font-semibold text-white text-lg hover:bg-indigo-700 transition-colors min-h-[56px]">
              ...
              return (
                <div className="flex flex-col gap-3">
                  {orderingItems.map((itemId, idx) => {
                    ...
                    if (!item) return null
                    return (
                      <div key={itemId} className="flex items-center gap-3 rounded-xl border-2 border-slate-600 bg-slate-800 px-4 py-3 min-h-[48px]">
                        <span className="text-slate-500 text-sm font-bold w-6 text-center shrink-0">{idx + 1}</span>
                        <span className="flex-1 font-medium">{item.text}</span>
                        {!isFeedback && (
                          <div className="flex flex-col gap-1 shrink-0">
                            <button disabled={idx === 0} onClick={() => setOrderingItems((prev) => { const a = [...prev]; [a[idx - 1], a[idx]] = [a[idx]!, a[idx - 1]!]; return a })} className="rounded bg-slate-700 px-2 py-0.5 text-xs disabled:opacity-20 hover:bg-slate-600">↑</button>
                            <button disabled={idx === orderingItems.length - 1} onClick={() => setOrderingItems((prev) => { const a = [...prev]; [a[idx], a[idx + 1]] = [a[idx + 1]!, a[idx]!]; return a })} className="rounded bg-slate-700 px-2 py-0.5 text-xs disabled:opacity-20 hover:bg-slate-600">↓</button>
                          </div>
                        )}
                      </div>
                    )
                  ...
                  {hq.imageUrl ? (
                    <>
                      <div
                        ...
                        style={{ cursor: isFeedback ? 'default' : 'crosshair', userSelect: 'none' }}
                        onClick={(e) => {
                      ...
                      >
                        <img src={hq.imageUrl} alt="" className="block w-full pointer-events-none" draggable={false} />
                        {hotspotClick && (
                          <div className="absolute rounded-full pointer-events-none" style={{
                            left: `${hotspotClick.x}%`, top: `${hotspotClick.y}%`,
                            transform: 'translate(-50%,-50%)', width: 28, height: 28,
                            border: `3px solid ${isFeedback ? (lastCorrect ? '#22c55e' : '#ef4444') : '#818cf8'}`,
                            background: isFeedback ? (lastCorrect ? 'rgba(34,197,94,0.3)' : 'rgba(239,68,68,0.3)') : 'rgba(129,140,248,0.3)',
                          }} />
                        )}
                        {isFeedback && hq.hotspots.filter((h) => h.isCorrect).map((h) => (
                          <div key={h.id} className="absolute rounded-full pointer-events-none" style={{
                            left: `${h.x}%`, top: `${h.y}%`,
                            transform: 'translate(-50%,-50%)',
                            width: `${h.radius * 2}%`, aspectRatio: '1',
                            border: '3px solid rgba(134,239,172,0.7)',
                            background: 'rgba(34,197,94,0.2)',
                          }} />
                        ))}
                      </div>
                      {!isFeedback && !hotspotClick && <p className="text-sm text-slate-400">Нажмите на правильную точку</p>}
                    </>
                  ) : (
            ...
            {isFeedback && (
              <AnswerFeedback
                correct={lastCorrect}
                {...(currentQ.explanation !== undefined && { explanation: currentQ.explanation })}
                className="mt-5"
              />
            )}

            <div className="mt-6">
              {phase === 'question' && currentQ.type !== 'hotspot' && currentQ.type !== 'matching' && (
                <button
                  disabled={
                    (currentQ.type === 'single_choice' && !selectedId) ||
                    (currentQ.type === 'true_false' && !selectedId) ||
                    (currentQ.type === 'multiple_choice' && selectedIds.length === 0) ||
                    (currentQ.type === 'ordering' && orderingItems.length === 0)
                  }
                  className="w-full rounded-xl bg-indigo-600 py-4 font-semibold text-white text-lg disabled:bg-slate-700 disabled:text-slate-400 transition-colors hover:bg-indigo-700 min-h-[56px]"
